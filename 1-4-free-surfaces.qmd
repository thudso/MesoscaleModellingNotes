...
jet). Problems in which surface tension is a
...
drop beads up (e.g. water on a lotus leaf).
...
state) when we look at raindrops on typical solids. In what follows, we will consider
...
corresponds to a drop hanging beneath a surface.
...
- Parametrise the free surface so that shapes are not restricted to
  ...
  drops'. To do so, one option is to switch to the
