...
\begin{document}
  \begin{tikzpicture}[thick]
    ...
    \fill[gray!30] (-2, 2) rectangle (-5, -2); % Left wall
    \fill[gray!30] (2, 2) rectangle (5, -2);  % Right wall

    % Draw the rectangular bar
    \fill[magenta!10] (-2, 0.5) rectangle (2, -0.5);
    \node[magenta] at (0,0) {Domain, $B$};

    % Traction free BCs
    \draw[very thick,blue] (-2,0.5) -- node[above] {$\bfS\bfn=\bf0$} (2,0.5);
    \draw[very thick,blue] (-2,-0.5) -- (2,-0.5);

    % Left displacement BC
    \draw[ultra thick,red] (-2,-0.5) -- (-2,0.5);
    \node[red] at (-3.5,0) {$\bfu=-\delta \bfe_1$};
