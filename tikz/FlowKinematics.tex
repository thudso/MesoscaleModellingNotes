  ...
  \begin{scope}
    ...
    \foreach \x in {-3,-2.5,-2,-1.5,-1,-0.5,0,0.5,1,1.5,2,2.5,3} {
      \foreach \y in {-3,-2.5,-2,-1.5,-1,-0.5,0,0.5,1,1.5,2,2.5,3} {
        % Define vector field
        \pgfmathsetmacro{\u}{1/(1+\y*\y/4)};
        \pgfmathsetmacro{\v}{-\y/(1+\y*\y/4)};
        % Normalize vector length
        \pgfmathsetmacro{\un}{0.4*\u};
        \pgfmathsetmacro{\vn}{0.4*\v};
        \draw[->, thick, opacity=0.2] (\x-\un/2,\y-\vn/2) -- ++(\un,\vn);
      }
    }

    \node[label={left:$\bfx$}] (A) at (-1,1) {};
    \node (B) at (-0.75,0) {};
    \draw[ultra thick] (A.center) -- (B.center) node[fill=white,below] {$\bfx+\delta\bfx$} ;
    
    \fill (A.center) circle (2pt);
    \draw[->,ultra thick, blue] (A.center) -- ($(A.center) + 1.2*(1.2/4,-1.2/4)$) node[fill=white,above right,inner sep=1pt] {$\bfv(\bfx)$};
    
    \fill (B.center) circle (2pt);
    \draw[->,ultra thick, blue] (B.center) -- ($(B.center) + 1.2*(0.4,0)$) node[fill=white,right,inner sep=1pt] {$\bfv(\bfx+\delta\bfx)$};
