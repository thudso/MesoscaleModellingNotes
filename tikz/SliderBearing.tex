...
\begin{tikzpicture}[scale=1.5]
  ...
  \draw[ultra thick] (-3.5,0.55) -- node[above right] {$y=L_y-\varepsilon^2 mx$} (3.5,-0.15);
  \fill[gray!20] % Fill below surface
  (-4, -1.5) -- (4, -1.5) -- (4,-3) -- (-4,-3) -- cycle;
  \draw[ultra thick] (-4,-1.5) --  node[above] {$y=0$} (4,-1.5);
  \draw[dashed] (3.5,-0.15) -- node[right] {$p=p_{\text{\text{atm}}}$} (3.5,-1.5) node[below] {$x=L_x$};
  \draw[dashed] (-3.5,0.55) -- node[left] {$p=p_{\text{atm}}$} (-3.5,-1.5);
  
  % Axes
  \draw[->,thick] (-4,-1.5) -- (4.5,-1.5) node[anchor=west] {$x$};
  \draw[->,thick] (-3.5,-3) -- (-3.5,1.0) node[anchor=east] {$y$};

  \draw[->,ultra thick] (-0.5,-2) --  node[below] {$U_b$} (0.5,-2);
  \draw[->,ultra thick] (-0.2,-0.7) -- node[below right] {$\mathbf{F}$?} (0.2,-0.3);
\end{tikzpicture}
\end{document}