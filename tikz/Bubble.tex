...
\begin{tikzpicture}
    ...
    \fill[cyan!20] (-3,-3) rectangle (5,3); % Light blue rectangle as background

    % Draw the white circle
    \fill[white] (0,0) circle (1.5); % White circle centered at the origin

    % Draw the inward-pointing normal vector
    \draw[->, ultra thick, black] 
        (1.06, 1.06) % Start point at the 2 o'clock position on the circle (approximated as a unit circle)
        -- (0.5, 0.5) % Arrow points inward
        node[anchor=north west] {$\bfn$}; % Label the vector

    \draw[<->, dashed] (-1.45,0) --node[above] {$R$} (0,0);
    % Draw the circle boundary
        \draw[thick] (0,0) circle (1.5); % Outline of the circle

        \draw (0.2,-0.7) -- (1.6,-1.5) node[anchor=west] {$p^+=p^-+\frac{2\sigma}{R}$};
        \node at (2,2) {$p^-$};
\end{tikzpicture}
\end{document}