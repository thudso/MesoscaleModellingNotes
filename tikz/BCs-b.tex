...
\begin{document}
  \begin{tikzpicture}
    ...
    \draw[thick] (-2,0) -- (2,0); % Horizontal boundary line from (-2,0) to (2,0)
    \fill[gray!10] (-2,0) -- (2,0) -- (2,1.5) -- (-2,1.5) -- cycle;
    
    % Draw hashing marks along the top of the boundary line
    \foreach \x in {-1.8, -1.4, -1, -0.6, -0.2, 0.2, 0.6, 1, 1.4, 1.8}
    \draw[thick] (\x,0) -- (\x+0.2,0.2);

    % Draw the right-pointing arrows above and below the boundary
    \draw[->, thick] (-1,0.5) -- node[anchor=south]{$\mathbf{u}_b$} (1,0.6); % Arrow above boundary
    \draw[->, thick] (-0.5,-0.6) -- node[anchor=north]{$\mathbf{u}$} (0.5,-0.5); % Arrow below boundary
  \end{tikzpicture}
\end{document}