...
\begin{tikzpicture}
  ...
  % phi
  \draw[red,domain=-1:0,samples=100,smooth]
  plot (\x,1+3*\x^2-2*\x^3) node[above right] {$\phi$};
  \draw[red,domain=0:1,samples=100,smooth]
  plot (\x,1-3*\x^2+2*\x^3);

  % psi
  \draw[blue,domain=-1:0,samples=100,smooth]
  plot (\x,{\x-2*\x^2+\x^3}) node[above left] {$\psi$};
  \draw[blue,domain=0:1,samples=100,smooth]
  plot (\x,{\x-2*\x^2+\x^3});
\end{tikzpicture}
\end{document}