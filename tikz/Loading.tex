...
\begin{tikzpicture}
  ...
  \coordinate (A) at (0,\x,\y);
  \coordinate (B) at (0,\x,-\y);
  \coordinate (C) at (0,-\x,-\y);
  \coordinate (D) at (0,-\x,\y);

  
  \fill[gray!30] (A) -- (B) -- (C) -- (D) -- cycle;
  \draw[thick,->] (O) -- (1,0,0) node[right] {$\bfn$};

  \draw[thick,->,magenta] (O) -- (0.8,1.1,-0.6) node[right] {$\bft$};
  \draw[thick,red,->] (O) -- node[left] {$\bft_s$} (0,1.1,-0.6);
  \draw[thick,blue,->] (0,1.1,-0.6) -- node[above] {$\bft_n$} (0.8,1.1,-0.6);
\end{scope}
\end{tikzpicture}
\end{document}