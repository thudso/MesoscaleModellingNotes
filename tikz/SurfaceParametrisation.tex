...
\begin{tikzpicture}[scale=1.5]
  ...
  plot(\x, {0.2*sin(3*\x r) + 0.2}) node[right] {$y = h(x,t)$};
  
  % Draw a normal vector pointing upwards
  \def\xpos{0.4};
  \filldraw[black] (\xpos, {0.2*sin(3*\xpos r) + 0.2}) circle (2pt);
  \draw[->, ultra thick] (\xpos, {0.2*sin(3*\xpos r) + 0.2}) -- ({\xpos-0.3*cos(3*\xpos r)}, {0.2*sin(3*\xpos r) + 0.2 +0.5}) node[above] {$\bfn$}; % Normal vector
  \node at (2,-0.8) {Gas};
  \node at (2,1) {Liquid};

  % Gravity
  \draw[ultra thick,->>] (-2,1.6) -- (-2,0.9) node[anchor=east] {$\mathbf{g}$};
  
  % Axes
  \draw[->,thick] (-3.5,-1.5) -- (3.8,-1.5) node[anchor=north] {$x$};
  \draw[->,thick] (-3.5,-1.5) -- (-3.5,2.0) node[anchor=east] {$y$};
\end{tikzpicture}
\end{document}