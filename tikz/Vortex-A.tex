...
\begin{tikzpicture}
  ...
  \draw[->] (-\R-0.3,0) -- (\R+0.3,0) node[below] {$x$};
  \draw[->] (0,-\R-0.3) -- (0,\R+0.3) node[left] {$y$};

  \draw[ultra thick,gray] (0,0) circle (\R);
  
  \foreach \r in {0.25, 0.5, 0.75} {
    \draw[dotted] (0,0) circle (\R*\r);
    \draw[->] (\R*\r,0) -- (\R*\r,\r);
  }
  \draw (0,0) -- (\R,1) node[above right] {$u_r=\Omega r$};
  ...
  % Eyes
  \fill (-0.1,0.075) circle (0.5pt);
  \fill (0.1,0.075) circle (0.5pt);
  % Smile (arc)
  \draw (-0.125,-0.05) arc[start angle=200, end angle=340, radius=0.125];
\end{scope}

\begin{scope}[shift={(-1.7,0)},rotate=90]
  % Face circle
    \fill[white] (0,0) circle (0.25);
    \draw (0,0) circle (0.25);
  % Eyes
  ...
  \draw[thick, red, ->] 
    (0,0) ++(-30:0.5) arc[start angle=-30, end angle=210, radius=0.5];
\end{scope}

\begin{scope}[shift={(0,-1.7)},rotate=180]
  % Face circle
    \fill[white] (0,0) circle (0.25);
    \draw (0,0) circle (0.25);
  % Eyes
